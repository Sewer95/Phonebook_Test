...
// Assert that button SignOut is present
  ...
  public void createAccountPositiveTestRefactor() {
      app.clickLoginLink();
      app.type(By.name("email"),"user_admin_new3@gmail.com");
    app.type(By.name("password"),"Password@1");
    app.click(By.name("registration"));
      app.isSignOutButtonPresent();
  }
  ...
  public void createExistedAccountNegativeTest() {
    SoftAssert softAssert = new SoftAssert();
      app.clickLoginLink();
      app.type(By.name("email"),"user_admin_new3@gmail.com"); //! Должен быть уже существующий юзер, чтобы перехватывать SoftAssert
    app.type(By.name("password"),"Password@1");
    app.click(By.name("registration"));
     ...
     */
    softAssert.assertTrue(app.isAlertPresent());
    //Assert.assertTrue(isElementPresent(By.xpath("//div[.='Registration failed with code 409']")));
    softAssert.assertTrue(app.isElementPresent(By.xpath("//div[.='Registration failed with code 409']")));
